...
export class PlayerMetrics {
  ...
  constructor() {
    this.registry.createGauge('darshan_player_info', 'Static player runtime metadata', ['app_version', 'runtime_mode'])
    this.registry.createCounter('darshan_player_heartbeat_total', 'Player heartbeat outcomes', ['result'])
    this.registry.createHistogram(
      'darshan_player_heartbeat_duration_seconds',
      'Player heartbeat duration in seconds',
      [0.1, 0.25, 0.5, 1, 2.5, 5, 10, 30],
      ['result']
    )
    this.registry.createCounter(
      'darshan_player_heartbeat_skips_total',
      'Skipped player heartbeat runs by reason',
      ['reason']
    )
    this.registry.createGauge(
      'darshan_player_last_successful_heartbeat_unixtime',
      'Unix timestamp of the last successful player heartbeat'
    )
    this.registry.createGauge('darshan_player_request_queue_items', 'Queued request count by category', ['category'])
    this.registry.createGauge('darshan_player_request_queue_bytes', 'Queued request bytes by category', ['category'])
    this.registry.createGauge('darshan_player_request_queue_budget_items', 'Request queue item budget by category', ['category'])
    this.registry.createGauge('darshan_player_request_queue_budget_bytes', 'Request queue byte budget by category', ['category'])
    this.registry.createGauge(
      'darshan_player_request_queue_oldest_age_seconds',
      'Age of the oldest queued request in seconds by category',
      ['category']
    )
    this.registry.createCounter(
      'darshan_player_request_queue_adjustments_total',
      'Cumulative request queue drops and compactions by category.',
      ['category', 'action']
    )
    this.registry.createGauge(
      'darshan_player_pop_replay_backlog',
      'Proof-of-play replay backlog by state.',
      ['state']
    )
    this.registry.createGauge(
      'darshan_player_pop_replay_budget',
      'Proof-of-play replay budget by state.',
      ['state']
    )
    this.registry.createCounter(
      'darshan_player_pop_replay_adjustments_total',
      'Cumulative proof-of-play replay drops and compactions.',
      ['action']
    )
    this.registry.createGauge('darshan_player_cache_bytes', 'Player cache capacity and usage in bytes', ['state'])
    this.registry.createGauge('darshan_player_cache_entries', 'Player cache entry counts', ['state'])
    this.registry.createGauge('darshan_player_state', 'One-hot player lifecycle state gauge', ['state'])
    this.registry.createCounter('darshan_player_command_total', 'Player command processing outcomes', [
      'command_type',
      'source',
      'result',
    ])
    this.registry.createCounter('darshan_player_command_ack_total', 'Player command acknowledgment outcomes', [
      'result',
    ])
    this.registry.createCounter('darshan_player_screenshot_upload_total', 'Player screenshot upload outcomes', [
      'result',
    ])
    this.registry.createCounter(
      'darshan_player_certificate_validation_total',
      'Player certificate validation and compatibility outcomes',
      ['result']
    )
    this.registry.createGauge('darshan_player_last_schedule_sync_unixtime', 'Unix timestamp of the last schedule sync')
    this.registry.createCounter(
      'darshan_player_metrics_scrape_failures_total',
      'Player metrics collector failures during scrape',
      ['collector']
    )
    this.registry.createGauge('darshan_player_system_cpu_usage_percent', 'Player host CPU usage percent')
    this.registry.createGauge('darshan_player_system_cpu_cores', 'Player host CPU core count')
    this.registry.createGauge('darshan_player_system_memory_bytes', 'Player host memory bytes', ['state'])
    this.registry.createGauge('darshan_player_system_disk_bytes', 'Player host disk bytes', ['state'])
    this.registry.createGauge(
      'darshan_player_system_temperature_celsius',
      'Player host reported temperature in Celsius'
    )
    this.registry.createGauge('darshan_player_system_uptime_seconds', 'Player host uptime in seconds')
    this.registry.createGauge('darshan_player_display_count', 'Attached display count')
    this.registry.createGauge('darshan_player_battery_percent', 'Player battery percentage when available')
    this.registry.createGauge('darshan_player_power_connected', 'Whether the player reports charging or AC power')
  ...
  safeRecordHeartbeat(result: HeartbeatResult, durationSeconds: number): void {
    this.captureMetricError('heartbeat', () => {
      this.registry.incCounter('darshan_player_heartbeat_total', { result })
      ...
      if (result === 'skipped_in_flight') {
        this.registry.incCounter('darshan_player_heartbeat_skips_total', { reason: 'in_flight' })
      }
      if (result === 'success') {
        this.registry.setGauge('darshan_player_last_successful_heartbeat_unixtime', {}, Math.floor(Date.now() / 1000))
      }
    })
  }
  ...
  setLastSuccessfulHeartbeat(timestamp: Date | number | string): void {
    this.captureMetricError('heartbeat', () => {
      ...
      if (unixTime > 0) {
        this.registry.setGauge('darshan_player_last_successful_heartbeat_unixtime', {}, unixTime)
      }
    })
  }
  ...
  setPlayerState(state: PlayerState): void {
    this.captureMetricError('player_state', () => {
      for (const candidate of PLAYER_STATES) {
        this.registry.setGauge('darshan_player_state', { state: candidate }, candidate === state ? 1 : 0)
      }
    })
  }
  ...
  recordCommandOutcome(commandType: CommandType, source: CommandSource, result: CommandOutcome): void {
    this.captureMetricError('command', () => {
      this.registry.incCounter('darshan_player_command_total', {
  ...
  recordCommandAck(result: CommandAckResult): void {
    this.captureMetricError('command_ack', () => {
      this.registry.incCounter('darshan_player_command_ack_total', { result })
    })
  }

  recordScreenshotUpload(result: ScreenshotUploadResult): void {
    this.captureMetricError('screenshot', () => {
      this.registry.incCounter('darshan_player_screenshot_upload_total', { result })
    })
  }

  recordCertificateValidation(result: CertificateValidationResult): void {
    this.captureMetricError('certificate_validation', () => {
      this.registry.incCounter('darshan_player_certificate_validation_total', { result })
    })
  }
  ...
  setLastScheduleSync(timestamp: Date | number | string = Date.now()): void {
    this.captureMetricError('schedule_sync', () => {
      const unixTime = this.toUnixSeconds(timestamp)
      if (unixTime > 0) {
        this.registry.setGauge('darshan_player_last_schedule_sync_unixtime', {}, unixTime)
      }
    })
  }
  ...
  private async captureCollector(collector: string, fn: () => void | Promise<void>): Promise<void> {
    try {
      ...
      this.registry.incCounter('darshan_player_metrics_scrape_failures_total', { collector })
  ...
  private updateSystemStats(systemStats: SystemStats): void {
    this.registry.setGauge('darshan_player_system_cpu_usage_percent', {}, systemStats.cpuUsage)
    this.registry.setGauge('darshan_player_system_cpu_cores', {}, systemStats.cpuCores)
    this.registry.setGauge('darshan_player_system_memory_bytes', { state: 'used' }, systemStats.memoryUsage)
    this.registry.setGauge('darshan_player_system_memory_bytes', { state: 'free' }, systemStats.memoryFree)
    this.registry.setGauge('darshan_player_system_memory_bytes', { state: 'total' }, systemStats.memoryTotal)
    this.registry.setGauge('darshan_player_system_disk_bytes', { state: 'used' }, systemStats.diskUsage)
    this.registry.setGauge('darshan_player_system_disk_bytes', { state: 'free' }, systemStats.diskFree)
    this.registry.setGauge('darshan_player_system_disk_bytes', { state: 'total' }, systemStats.diskTotal)
    this.registry.setGauge('darshan_player_system_uptime_seconds', {}, systemStats.uptime)
    this.registry.setGauge('darshan_player_display_count', {}, systemStats.displayCount)
    this.registry.setGauge(
      'darshan_player_battery_percent',
      {},
      typeof systemStats.batteryPercent === 'number' ? systemStats.batteryPercent : 0
    )
    this.registry.setGauge(
      'darshan_player_power_connected',
      {},
      systemStats.isCharging === true || systemStats.powerSource === 'AC' ? 1 : 0
    )
    if (typeof systemStats.temperature === 'number') {
      this.registry.setGauge('darshan_player_system_temperature_celsius', {}, systemStats.temperature)
    }
  }

  private updateCacheStats(cacheStats: CacheStats & { usagePercent: number; itemCount: number }): void {
    this.registry.setGauge('darshan_player_cache_bytes', { state: 'total' }, cacheStats.totalBytes)
    this.registry.setGauge('darshan_player_cache_bytes', { state: 'used' }, cacheStats.usedBytes)
    this.registry.setGauge('darshan_player_cache_bytes', { state: 'free' }, cacheStats.freeBytes)
    this.registry.setGauge('darshan_player_cache_entries', { state: 'all' }, cacheStats.entryCount)
    this.registry.setGauge('darshan_player_cache_entries', { state: 'quarantined' }, cacheStats.quarantinedCount)
  }
  ...
  private updateQueueStats(): void {
    ...
    this.registry.setGauge('darshan_player_request_queue_items', { category: 'all' }, stats.pendingItems)
    this.registry.setGauge('darshan_player_request_queue_bytes', { category: 'all' }, stats.pendingBytes)
    this.registry.setGauge('darshan_player_request_queue_budget_items', { category: 'all' }, budgets.totalMaxItems)
    this.registry.setGauge('darshan_player_request_queue_budget_bytes', { category: 'all' }, budgets.totalMaxBytes)

    for (const category of Object.keys(stats.categories)) {
      const categoryKey = category as keyof typeof stats.categories
      this.registry.setGauge(
        'darshan_player_request_queue_items',
        { category },
        stats.categories[categoryKey].pendingItems
      )
      this.registry.setGauge(
        'darshan_player_request_queue_bytes',
        { category },
        stats.categories[categoryKey].pendingBytes
      )
      this.registry.setGauge(
        'darshan_player_request_queue_budget_items',
        { category },
        budgets.categories[categoryKey].maxItems
      )
      this.registry.setGauge(
        'darshan_player_request_queue_budget_bytes',
        { category },
    ...
    for (const [category, oldestAge] of Object.entries(oldestAgeByCategory)) {
      this.registry.setGauge('darshan_player_request_queue_oldest_age_seconds', { category }, oldestAge)
    }
  }
  ...
  private updateProofOfPlayReplayStats(): void {
    ...
    this.registry.setGauge('darshan_player_pop_replay_backlog', { state: 'buffer_items' }, stats.bufferItems)
    this.registry.setGauge('darshan_player_pop_replay_backlog', { state: 'buffer_bytes' }, stats.bufferBytes)
    this.registry.setGauge('darshan_player_pop_replay_backlog', { state: 'spool_files' }, stats.spoolFiles)
    this.registry.setGauge('darshan_player_pop_replay_backlog', { state: 'spool_bytes' }, stats.spoolBytes)

    this.registry.setGauge('darshan_player_pop_replay_budget', { state: 'buffer_items' }, budgets.maxBufferEvents)
    this.registry.setGauge('darshan_player_pop_replay_budget', { state: 'buffer_bytes' }, budgets.maxBufferBytes)
    this.registry.setGauge('darshan_player_pop_replay_budget', { state: 'spool_files' }, budgets.maxSpoolFiles)
    this.registry.setGauge('darshan_player_pop_replay_budget', { state: 'spool_bytes' }, budgets.maxSpoolBytes)
    this.registry.setGauge(
      'darshan_player_pop_replay_budget',
