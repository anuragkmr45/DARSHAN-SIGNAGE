...
const AUTOSTART_DESKTOP_ENTRY = 'darshan-player.desktop'
...
function shouldManageAutostart() {
  if (process.env['DARSHAN_AUTOSTART_ENABLED'] === 'false' || process.env['HEXMON_AUTOSTART_ENABLED'] === 'false') {
    return false
  }
...
function buildLinuxDesktopEntry() {
  ...
  return [
    ...
    'Version=1.0',
    'Name=DARSHAN Player',
    'Comment=DARSHAN digital signage player',
    `Exec=${executable}`,
    'Terminal=false',
