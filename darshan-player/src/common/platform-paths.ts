...
function getFallbackRuntimeRoot(platform: NodeJS.Platform): string {
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env['APPDATA'] || path.join(homeDir, 'AppData', 'Roaming'), 'DARSHAN Player')
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'DARSHAN Player')
    default:
      return path.join(homeDir, '.config', 'darshan-player')
  }
}

export function resolveRuntimePaths(appInstance?: ElectronApp): RuntimePaths {
  const configOverride =
    getEnvOverride('DARSHAN_CONFIG_PATH') || getEnvOverride('SIGNAGE_CONFIG_PATH') || getEnvOverride('HEXMON_CONFIG_PATH')
  const runtimeRoot =
    getEnvOverride('DARSHAN_RUNTIME_ROOT') ||
  ...
  const cachePath = getEnvOverride('DARSHAN_CACHE_PATH') || getEnvOverride('HEXMON_CACHE_PATH') || path.join(runtimeRoot, 'cache')
  const certDir =
    getEnvOverride('DARSHAN_MTLS_CERT_DIR') || getEnvOverride('HEXMON_MTLS_CERT_DIR') || path.join(runtimeRoot, 'certs')
  ...
  return {
    ...
    certDir,
    legacyLinux: {
      configPath: '/etc/darshan/config.json',
      cachePath: '/var/cache/darshan',
      certDir: '/var/lib/darshan/certs',
    },
  }
}
