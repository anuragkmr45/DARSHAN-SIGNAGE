...
# Assists with pairing a device to the DARSHAN backend

set -e

echo "DARSHAN Player - Device Pairing"
echo "======================================"
echo ""
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Usage: sudo darshan-pair-device"
    exit 1
fi
...
# Check configuration
CONFIG_FILE="/etc/darshan/config.json"
if [ ! -f "$CONFIG_FILE" ]; then
    echo "Error: Configuration file not found at $CONFIG_FILE"
    echo "Please run: sudo cp /usr/share/darshan/config.example.json $CONFIG_FILE"
    exit 1
fi
...
CERT_DIR="/var/lib/darshan/certs"
mkdir -p "$CERT_DIR"
chmod 700 "$CERT_DIR"
...
if echo "$RESPONSE" | grep -q "device_id"; then
    ...
    chown -R darshan:darshan "$CERT_DIR"
