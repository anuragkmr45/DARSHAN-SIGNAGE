...
This guide will help you get the DARSHAN Player up and running quickly.

For on-prem deployment, do not start with this legacy quick-start alone. Use `docs/runbooks/onprem-qa-setup.md` for QA or `docs/runbooks/onprem-production-setup.md` for production. The supported on-prem profile is IP-only and points players directly at `http://<backend-ip>:3000`.
...
Output: `build/darshan-player_1.0.0_amd64.deb`
...
Output: `build/darshan-player_1.0.0_x86_64.AppImage`
...
1. `$DARSHAN_CONFIG_PATH`
2. `$SIGNAGE_CONFIG_PATH`
3. `$HEXMON_CONFIG_PATH` legacy alias
4. the per-user Electron app-data config location
5. `./config.json`
...
```bash
chmod 755 ~/.config/darshan
chmod 600 ~/.config/darshan/config.json
```
