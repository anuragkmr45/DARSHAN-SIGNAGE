...
This audit answers the question: can `darshan-player` be packaged, installed, launched, paired, run in kiosk mode, autostart, play content, collect diagnostics, and degrade safely on Windows without Linux assumptions leaking into runtime behavior?
...
2. Legacy Linux docs and shell scripts are still prominent enough to mislead operators.
   ...
   Why it matters:
   - these paths still reference `systemd`, `/etc/darshan`, `/var/lib/darshan`, `/var/cache/darshan`, `.deb`, `AppImage`, `xrandr`, and `xset` as if they were primary operator workflows
   Current status:
...
| `scripts/darshan-player.service` | Linux-only and legacy | Systemd unit; not part of supported Windows flow |
