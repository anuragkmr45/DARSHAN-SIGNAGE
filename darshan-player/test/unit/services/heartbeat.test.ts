...
describe('Heartbeat Service', () => {
  ...
  it('coalesces immediate and scheduled heartbeats so only one POST stays in flight', async () => {
    ...
    expect(metrics).to.contain('darshan_player_heartbeat_total{result="skipped_in_flight"} 1')
    expect(metrics).to.contain('darshan_player_heartbeat_skips_total{reason="in_flight"} 1')
