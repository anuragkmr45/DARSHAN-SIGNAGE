...
describe('Health Server', () => {
  ...
  it('serves Prometheus metrics from the maintainable player registry', async () => {
    ...
    expect(response.body).to.contain('darshan_player_info')
    expect(response.body).to.contain('darshan_player_heartbeat_total{result="success"} 1')
    expect(response.body).to.contain('darshan_player_request_queue_items{category="all"} 2')
    expect(response.body).to.contain('darshan_player_request_queue_budget_bytes{category="all"} 524288')
    expect(response.body).to.contain('darshan_player_request_queue_adjustments_total{category="heartbeat",action="dropped"} 1')
    expect(response.body).to.contain('darshan_player_cache_bytes{state="used"} 400')
    expect(response.body).to.contain('darshan_player_pop_replay_backlog{state="spool_files"} 2')
    expect(response.body).to.contain('darshan_player_pop_replay_adjustments_total{action="compacted"} 2')
    expect(response.body).to.contain('darshan_player_state{state="PAIRED_RUNTIME"} 1')
    expect(response.body).to.contain('darshan_player_command_total{command_type="REFRESH",source="heartbeat",result="success"} 1')
    expect(response.body).to.contain('darshan_player_screenshot_upload_total{result="queued"} 1')
  })
