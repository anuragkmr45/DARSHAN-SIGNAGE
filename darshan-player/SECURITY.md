...
The DARSHAN Player is designed with security as a top priority. This document outlines the security features, best practices, and vulnerability reporting procedures.
...
**Key Files:**
- `/var/lib/darshan/certs/client.key` - Private key (0600 permissions)
- `/var/lib/darshan/certs/client.crt` - Client certificate (0600 permissions)
- `/var/lib/darshan/certs/ca.crt` - CA certificate (0600 permissions)
...
```bash
/var/lib/darshan/certs/     # 0700 (drwx------)
  client.key               # 0600 (-rw-------)
  client.crt               # 0600 (-rw-------)
  ca.crt                   # 0600 (-rw-------)
```
...
**Configuration:**
```bash
/etc/darshan/               # 0750 (drwxr-x---)
  config.json              # 0640 (-rw-r-----)
```

**Cache:**
```bash
/var/cache/darshan/         # 0755 (drwxr-xr-x)
...
2. **Access Control:**
   - Run service as dedicated `darshan` user
   - Limit sudo access
...
The DARSHAN Player follows these security standards:
- OWASP Top 10
- CWE/SANS Top 25
...
- Email: security@darshan.com
- Documentation: https://docs.darshan.com/security
- Support: support@darshan.com
