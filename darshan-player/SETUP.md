...
Complete setup instructions for the DARSHAN Player in both development and production environments.
...
```bash
# Windows (PowerShell)
$env:DARSHAN_CONFIG_PATH = "config.dev.json"

# Linux/macOS
export DARSHAN_CONFIG_PATH=config.dev.json
```
...
# Check logs
cat dev-cache/logs/darshan-*.log
...
# Or use curl
curl -O https://releases.darshan.com/darshan-player_1.0.0_amd64.deb
```
...
```bash
sudo chown root:darshan /etc/darshan/config.json
...
sudo openssl req -new \
  ...
  -subj "/CN=$(hostname)/O=DARSHAN/C=US"

# 3. Submit CSR to admin dashboard and download certificates

# 4. Save certificates
sudo nano /var/lib/darshan/certs/client.crt  # Paste client certificate
sudo nano /var/lib/darshan/certs/ca.crt      # Paste CA certificate

# 5. Set permissions
sudo chmod 600 /var/lib/darshan/certs/*.crt
...
● darshan-player.service - DARSHAN Player
   ...
   Main PID: 1234 (darshan-signage-)
      Tasks: 15
     Memory: 250.0M
     CGroup: /system.slice/darshan-player.service
             └─1234 /usr/bin/darshan-player
```
...
# Logs since boot
sudo journalctl -u darshan-player -b
...
# Cache directory
sudo chmod 755 /var/cache/darshan
...
```bash
# Create backup
sudo tar -czf darshan-backup-$(date +%Y%m%d).tar.gz \
  /etc/darshan/ \
  /var/lib/darshan/certs/

# Store backup securely
sudo mv darshan-backup-*.tar.gz /backup/location/
```
...
```bash
# Extract backup
sudo tar -xzf darshan-backup-20250105.tar.gz -C /
...
2. Install DARSHAN Player package
...
```bash
# Check configuration
sudo cat /etc/darshan/config.json | jq .

# Check permissions
ls -la /var/lib/darshan/certs/
...
- Deployment guide: `docs/runbooks/player-deployment.md`
