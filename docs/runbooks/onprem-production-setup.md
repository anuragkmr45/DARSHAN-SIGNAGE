# DARSHAN Production Setup Guide

This is the primary production deployment runbook for support teams deploying DARSHAN on-prem.
...
```bash
./stop.sh
ln -sfn "/opt/darshan/${SITE_NAME}/releases/<previous-release-id>/<folder>" "/opt/darshan/${SITE_NAME}/current"
./start.sh
./health-check.sh
```
...
```bash
cd /opt/darshan/site-a/releases/2026-03-30-r1/<folder>
./load-images.sh
./start.sh
./health-check.sh
```
