...
| CONFIG_ARCHITECTURE_PENDING | High | Backend mitigated by CONFIG-1; player mitigated by CONFIG-2/CONFIG-2.1/CONFIG-2.2/CONFIG-2.3; CMS mitigated locally by CONFIG-3; profile sets mitigated by CONFIG-4; runtime evidence open | CONFIG-1 added optional backend JSON config loading with env overrides and redacted diagnostics. CONFIG-2 added optional player-specific JSON site config loading, environment/deployment headers, and redacted player diagnostics while preserving runtime config behavior. CONFIG-2.1 redacts credentialed URL userinfo, query strings, and fragments from player config diagnostics. CONFIG-2.2 extended player URL redaction to pairing diagnostics, doctor output, support-bundle diagnostics, HTTP/WebSocket/startup logs, request-queue logs, and log-shipping URL logs but was independently found incomplete. CONFIG-2.3 patches remaining log-shipper, renderer webpage, renderer-to-main, support/log-shipment text bundle, and logger message emissions, and restores request-queue byte accounting to raw runtime URL data. CONFIG-3 adds optional CMS `/config/app-config.json` browser runtime config with Vite fallback and secret-looking key/credentialed URL rejection. CONFIG-4 adds dev/QA/prod on-prem config profile sets and static validation script. | Independently verify CONFIG-2.3 through CONFIG-4. Keep secrets in env/secrets files and existing env compatibility during migration. Browser/on-prem runtime evidence remains required. |
| CONFIG_RUNTIME_VALIDATION_PENDING | High | Open | Local listeners were discovered on expected ports, but CONFIG-5A health checks to `192.168.0.5` and localhost backend/CMS/media/Prometheus candidates failed; CONFIG-5A found Node `v24.12.0` only, no Node 20 runtime, no usable packaged player artifact, missing Valkey/Redis CLI tools, and all required `ONPREM_*` plus `NODE20_PATH` inputs missing. `scripts/verify/check-config5-runtime-readiness.sh --dry-run` reports `MISSING_INPUT`; no runtime config evidence was claimed. | Fill `docs/implementation/config-phase-5-runtime-inputs.template.env` into a local ignored `.local/config5-runtime.env`, provide valid on-prem QA endpoints, Node 20 runtime, packaged player target, player runtime roots, TLS/CA details, and pairing workflow/simulator credentials, then rerun browser/on-prem evidence using a CONFIG-4 profile set. |
