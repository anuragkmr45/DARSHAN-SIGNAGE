...
import { appendLegacySignhexMetricAliases, getObservabilityRegistry, observeHttpRequest } from '@/observability/metrics';

const REQUEST_START_KEY = Symbol.for('darshan.observability.request_start_ns');
...
export async function registerObservabilityHttp(fastify: FastifyInstance) {
  ...
  fastify.get('/metrics', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    reply.send(appendLegacySignhexMetricAliases(await registry.metrics()));
  });
}
