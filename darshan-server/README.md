...
For on-prem runtime-bundle deployment, start with the canonical runbooks in the `DARSHAN monorepo root` repo:
...
See `docs/runbooks/onprem-bundle-builder.md` for the unified QA + production runtime bundle workflow.
...
- generate the source-free backend deploy package from `scripts/export/package-server.sh`
...
```bash
git clone https://github.com/DARSHAN/darshan-server
cd darshan-server
cp .env.example .env
npm install
...
```bash
docker build -t darshan-api .
```

Run it:

```bash
docker run --env-file .env -p 3000:3000 darshan-api
```
...
- `PG_DUMP_PATH`
- `TAR_PATH`
- `DARSHAN_WEBPAGE_CAPTURE_EXECUTABLE_PATH`
- `APP_PUBLIC_BASE_URL`
- `ENABLE_SWAGGER_UI`
...
Set `DARSHAN_WEBPAGE_CAPTURE_EXECUTABLE_PATH` only when Chromium is installed outside the standard Playwright location. `HEXMON_WEBPAGE_CAPTURE_EXECUTABLE_PATH` remains a legacy alias for one release.
...
- `npm run start:api` and `npm run start:worker` are the explicit split-runtime entrypoints. You can also set `DARSHAN_PROCESS_ROLE=api|worker|all`; `HEXMON_PROCESS_ROLE` remains a legacy alias for one release.
