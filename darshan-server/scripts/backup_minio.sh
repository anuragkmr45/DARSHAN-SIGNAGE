...
BACKUP_FILE="$BACKUP_DIR/darshan_minio_$TIMESTAMP.tar.gz"
...
mc alias set darshan "$PROTOCOL://$MINIO_ENDPOINT:$MINIO_PORT" "$MINIO_ACCESS_KEY" "$MINIO_SECRET_KEY" --api S3v4

# Backup each bucket
for bucket in "${BUCKETS[@]}"; do
    echo "Backing up bucket: $bucket"
    mc mirror "darshan/$bucket" "$TEMP_DIR/$bucket" --quiet || true
done
