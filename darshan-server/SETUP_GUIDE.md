# DARSHAN Backend - Complete Setup & Deployment Guide

> Support note: for current on-prem bundle deployments, start with `docs/runbooks/onprem-qa-setup.md` for QA or `docs/runbooks/onprem-production-setup.md` for production. This older guide still contains legacy examples and is not the primary supported bundle path.

**Version:** 1.0.0  
**Last Updated:** 2025-11-05

This comprehensive guide will walk you through setting up the DARSHAN Backend from scratch, whether you're setting up a development environment or deploying to production.
...
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/darshan

# Note: Change 'postgres' password in production!
# Example for production: postgresql://darshan_user:SecurePassword123@db.example.com:5432/darshan_prod
...
```
[+] Running 2/2
 ✔ Container darshan-postgres  Started
 ✔ Container darshan-minio     Started
```
...
# Should show:
# - darshan-postgres (port 5432)
# - darshan-minio (ports 9000, 9001)
```
...
CREATE USER darshan_user WITH PASSWORD 'your_password';

-- Grant privileges
GRANT ALL PRIVILEGES ON DATABASE darshan TO darshan_user;
...
4. Open pgAdmin or use psql to create the `darshan` database

**Update `.env` file:**
```bash
DATABASE_URL=postgresql://postgres:your_password@localhost:5432/darshan
```
...
[INFO] Creating admin user: admin@darshan.local
...
DATABASE_URL=postgresql://darshan_prod:STRONG_PASSWORD_HERE@db.example.com:5432/darshan_prod
...
pg_dump -U postgres -h localhost darshan_prod > backup_$(date +%Y%m%d_%H%M%S).sql
...
# View logs
pm2 logs darshan-api

# Monitor resources
pm2 monit

# Restart application
pm2 restart darshan-api

# Stop application
pm2 stop darshan-api
```
...
Create `/etc/systemd/system/darshan-signage.service`:

```ini
[Unit]
Description=DARSHAN API
After=network.target postgresql.service

[Service]
Type=simple
User=darshan
WorkingDirectory=/opt/darshan-signage/server
Environment=NODE_ENV=production
EnvironmentFile=/opt/darshan-signage/server/.env
ExecStart=/usr/bin/node /opt/darshan-signage/server/dist/index.js
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal
SyslogIdentifier=darshan-signage
...
sudo systemctl enable darshan-signage

# Start service
sudo systemctl start darshan-signage

# Check status
sudo systemctl status darshan-signage

# View logs
sudo journalctl -u darshan-signage -f
```
...
docker build -t darshan-api:latest .
```
...
Create `/etc/nginx/sites-available/darshan-signage`:
...
server {
    ...
    # Logging
    access_log /var/log/nginx/darshan-access.log;
    error_log /var/log/nginx/darshan-error.log;
}
```
...
sudo ln -s /etc/nginx/sites-available/darshan-signage /etc/nginx/sites-enabled/
...
CREATE USER darshan_app WITH PASSWORD 'strong_password';

-- Grant only necessary privileges
GRANT CONNECT ON DATABASE darshan_prod TO darshan_app;
GRANT USAGE ON SCHEMA public TO darshan_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO darshan_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO darshan_app;

-- Revoke public access
REVOKE ALL ON DATABASE darshan_prod FROM PUBLIC;
```
...
4. **Test connection manually:**
   ```bash
   psql -h localhost -U postgres -d darshan
...
4. **Verify proxy_pass URL:**
   ```bash
   cat /etc/nginx/sites-available/darshan-signage | grep proxy_pass
...
2. **Check for connection leaks:**
   ...
   SELECT count(*) FROM pg_stat_activity WHERE datname = 'darshan_prod';
   
   -- Kill idle connections
   SELECT pg_terminate_backend(pid)
   FROM pg_stat_activity
   WHERE datname = 'darshan_prod'
   AND state = 'idle'
...
1. **Check the logs:**
   ...
   # Nginx logs
   sudo tail -f /var/log/nginx/darshan-error.log
   ```
...
docker exec -it darshan-postgres psql -U postgres  # Access database
```
...
You've successfully set up the DARSHAN Backend!
